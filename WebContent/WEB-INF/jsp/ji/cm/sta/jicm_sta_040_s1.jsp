...
/* 프로그램 이름     : jista_040_s1
/* 소스파일 이름     : jista_040_s1.jsp
/* 설명              : 다운로드
/* 버전              : 1.0.0
/* 최초 작성자       : 윤성종
/* 최초 작성일자     : 2018-03-12
...
$(document).ready(function() {
	...
	var cols =[
	           	...
	           	,{label:'다운건수'  ,name:'excel_down_cnt'       ,index:'excel_down_cnt'    ,align:'center'  ,width:'100'  ,hidden:false  ,sortable:false}
		      ];
	...
	$("#btn_excel_down").on("click",function(e){
		...
		excelDownload("jqGrid", "A", $("#start_ymd").val()+"~"+$("#end_ymd").val()+"_다운로드", "<c:url value='/cmsmain.do'/>");
...
function addChart(res_data){
	...
	for(hi=0;hi<rows.length;hi++){
		...
		tempItems1.push(rows[hi].menu_nm, rows[hi].excel_down_cnt);
...
function addHichartGraph(data){
	...
	for (i in data.rows) {
		...
		var obj = { "name" : data.rows[i].menu_nm, "y" : data.rows[i].excel_down_cnt };
		arrData[i] = obj;
	}
	
    // 카테고리 생성
    for (i in data.rows) {
        var obj = data.rows[i].menu_nm;
        arrCategories[i] = obj;
    }
