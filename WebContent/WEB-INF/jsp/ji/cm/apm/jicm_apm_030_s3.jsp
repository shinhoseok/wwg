...
/*=================================================================================*/
/* 시스템            : 결재관리
/* 프로그램 아이디   : /WEB-INF/jsp/ji/cm/apm/jicm_apm_030_s3.jsp 
/* 프로그램 이름     : jicm_apm_030_s3  
/* 소스파일 이름     : jicm_apm_030_s3.jsp
/* 설명              : 결재현황 - 완료함(사용안함)
/* 버전              : 1.0.0
/* 최초 작성자       : 이금용
/* 최초 작성일자     : 2015-06-11
...
$(function(){
	//결재현황 리스트
		var cols =[
		         {label:'번호'          	,index:'row_seq'     		,name:'row_seq'         ,align:'center' 	,width : '50',   hidden:false,frozen:true }
				,{label:'결재번호' 	 	,index:'real_stat_seqno'    ,name:'real_stat_seqno' ,align:'center' 	,width : '80', 	 hidden:false,frozen:true }
				,{label:'결재요청명' 	 	,index:'aprv_req_nm'     	,name:'aprv_req_nm'     ,align:'center' 	,width : '120',	 hidden:false, formatter : fm_moveJob }
				...
				,{label:'업무화면상태'	 	,index:'job_pstate'   		,name:'job_pstate'    	,align:'center' 	,width : '100',  hidden:true }				    ,{label:'업무화면파라메터명'	,index:'job_param_nm'   ,name:'job_param_nm'    ,align:'center' 	,width : '100', 	hidden:true }
...
function moveJob(rowId) {
	...
	//alert(ret.job_param_nm+"::"+ret.rtn_param_nm);
	//$("#pcode").val(ret.menu_cd);
	//$("#saprv_req_job_no").val(ret.aprv_req_job_no);
	var url = '';
	...
	if(ret.job_pstate=="" && ret.rtn_pstate==""){
		url = url+'&pstate=UF';
	}else{
		if(ret.job_pstate!=""){
			url = url+'&pstate='+ret.job_pstate;
		}else{
  ...
  <table class="tbl_search">
    ...
    <tr>
			...
			<td colspan="3">
	            ...
	            <input type="text" name="sst_aprv_req_ymd" id="sst_aprv_req_ymd"  value="<%=st_aprv_req_ymd.substring(0,4)+"-"+st_aprv_req_ymd.substring(4,6)+"-"+st_aprv_req_ymd.substring(6,8)%>" readonly onClick="Calendar_D(this);" title="결재요청일자 검색시작일 입력"  /> 
	            ...
	            <input type="text" name="sed_aprv_req_ymd" id="sed_aprv_req_ymd" value="<%=curDate.substring(0,4)+"-"+curDate.substring(4,6)+"-"+curDate.substring(6,8)%>" readonly onClick="Calendar_D(this);" title="결재요청일자 검색종료일 입력" />
...
<div class="section mT10">
	<h3 class="fL"><span>&#8226;</span><p id="tab_inner_title">완료함 리스트</p></h3>
	<p class="fR">
