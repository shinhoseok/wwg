...
<% 
String s_bs_aprv_ln_seqno = "";  	//기본결재선 일련번호
String s_default_aprv = "";			//기본결재선 여부
String s_bs_aprv_ln_nm = ""; 		//결재선 명
String s_base_aprv_line_desc = ""; 	//비고

Map map1 = new HashMap();
map1 = (Map)rtn_Map.get("rows");

if(viewMap!=null){
	s_bs_aprv_ln_seqno = CM_Util.nullToEmpty((String)viewMap.get("bs_aprv_ln_seqno"));  	//기본결재선 일련번호
	s_bs_aprv_ln_nm = CM_Util.nullToEmpty((String)viewMap.get("bs_aprv_ln_nm")); 			//결재선 명
	s_default_aprv = CM_Util.nullToEmpty((String)viewMap.get("default_aprv")); 				//기본결재선 여부
	s_base_aprv_line_desc = CM_Util.nullToEmpty((String)viewMap.get("base_aprv_line_desc")); //비고
}
...
$(function(){
	...
	jQuery("#jqGrid").jqGrid('gridResize',{});
		...
		$("#btn_delete").click(function(e){
			if(confirm("삭제하시겠습니까?")){
				$("#pstate").val("D");
				var params = jQuery("#uffrm").serialize();
		...
		$("#btn_grid_add").click(function(e){
			...
			for(i=0;i < ids.length;i++) {
				...
				//console.log('ids.length=='+(ids.length-1)+' i== '+i+' addCnt=='+addCnt)
...
<form name="cffrm" id="cffrm" method="post"  onSubmit='return false;' >        
	...
	<input type="hidden" name="bs_aprv_ln_seqno" id="bs_aprv_ln_seqno" value="<%=s_bs_aprv_ln_seqno %>" title="결재선일련번호" />
	...
	<input type="hidden" name="delkey" id="delkey" value="" />
	  ...
	  <div class="board_A0_W">
	  	<h3 class="fL mB5"><span>&#8226;</span><%=M_PATH_STR.substring(M_PATH_STR.lastIndexOf("::")+2)%> 수정</h3>
	    <table>
	      <colgroup>
		   		<col width="15%" />
		   		<col width="55%" />
	      ...
	      <tr>
	    	...
	    	<td colspan="3">
	    		<input type="text" name="bs_aprv_ln_nm" id="bs_aprv_ln_nm" maxlength="100" class="input_type01 w_95p" value="<%=s_bs_aprv_ln_nm %>" />
	    	</td>
<%-- 	    	<th>기본결재선</th>
	    	<td class="w20">
	    		<input type="checkbox" name="default_aprv" id="default_aprv" value="Y" <%if(s_default_aprv.equals("Y")){ %> checked<%}%> />
	    	</td> --%>
	      </tr>
	      <tr>
	       	<th scope="row">비고</th>
	       	<td colspan="4">
				<input type="text" name="base_aprv_line_desc" id="base_aprv_line_desc" maxlength="2000" class="input_type01 w_95p" value="<%=s_base_aprv_line_desc %>" />
	      	</td>
	    </table>
	...
	<div class="button a_r mat_15">
		<a href="#" class="btn_1 size_n" id="btn_delete">삭제</a>
