...
public class CmsDs2Dao extends EgovAbstractDAO {
	protected Logger log = Logger.getLogger(CmsDs2Dao.class); //현재 클래스 이름을 Logger에 등록
...
//		SimpleJdbcTemplate template = new SimpleJdbcTemplate(super.getDataSource());
	...
	public String getPaginationSqlOracle(String innerQuery, int pageIndex, int pageSize) {
		...
		sql.append("\n ) INNER_TABLE )  WHERE  ROW_SEQ <= ? AND ROW_SEQ BETWEEN ? AND ? ");
		return sql.toString();
	}
