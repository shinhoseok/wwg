/**
 * SmartEditor2 NAVER_Library:SE2.9.0
 * Copyright NAVER Corp. Licensed under LGPL v2.1
 * See license text at https://github.com/Lee-Jung-Hoon/smarteditor2/blob/master/LICENSE.md
 */
...
//		jindo.$Element(this.toolbarArea).addClass("off");
	...
	$ON_ENABLE_RESIZE_UI : function(htOptions){
		if(this.nUIStatus === 2){
			return;
		}
...
//		jindo.$Element(this.toolbarArea).addClass("off");
	...
	_getAffectedElements : function(el){
		...
		if(!el.bSE2_MDCancelled){
			...
			var aBtns = jindo.$ElementList('BUTTON');
...
//	iMinUndoInterval : 50,
	...
	$init : function(iframe){
		...
		this.sBlankPageURL = "/KomipoWwg/se290/smart_editor2_inputarea.html";
		this.sBlankPageURL_EmulateIE7 = "/KomipoWwg/se290/smart_editor2_inputarea_ie8.html";
	...
	$ON_CHANGE_EDITING_MODE : function(sMode, bNoFocus){
		// editor/html/text 모드 변경 시 이미지 사이즈 변경 레이어 숨기기 위해 객체 선언
		var ImgSizeRatioKeeper = new nhn.husky.SE2M_ImgSizeRatioKeeper();
		if(sMode === this.sMode){
		...
		ImgSizeRatioKeeper._hideLayer(); //이미지 사이즈 변경 레이어 숨김
	},
...
//				sCssUrl += "/smart_editor2_in.css";
				sCssUrl = "/KomipoWwg/se290/css/smart_editor2_in.css";
...
nhn.husky.SE_EditingAreaVerticalResizer = jindo.$Class({
	...
	/**
	 * [customize] ENABLE_RESIZE_UI 메시지가 발생하면 타 기능은 비활성화하며 입력창 길이 조절기능만 활성화한다.
	 */
	$ON_ENABLE_RESIZE_UI : function(){
...
nhn.husky.SE2M_EditingModeChanger = jindo.$Class({
	...
	// $ON_DISABLE_ALL_UI 기능을 그대로 보전하기 위해 분리
	$ON_ENABLE_RESIZE_UI : function(htOptions){
		htOptions = htOptions || {};
		var waExceptions = jindo.$A(htOptions.aExceptions || []);
	...
	$ON_CHANGE_EDITING_MODE : function(sMode){
		if(sMode == "HTMLSrc"){
			...
			// $ON_DISABLE_ALL_UI 기능을 그대로 보전하기 위해 분리 - 모든 기능 비활성화
			// this.oApp.exec("DISABLE_ALL_UI", [{aExceptions:["mode_switcher"]}]);
			this.oApp.exec("ENABLE_RESIZE_UI", [{aExceptions:["mode_switcher"]}]);
		} else if (sMode == 'TEXT') {
			this.welWYSIWYGButtonLi.removeClass("active");
			this.welHTMLSrcButtonLi.removeClass("active");
			this.welTEXTButtonLi.addClass("active");
...
nhn.husky.SE2M_ImgSizeRatioKeeper = jindo.$Class({
	...
	$init : function(elAppContainer){
		this.elAppContainer = elAppContainer;
		this.imgUtil = jindo.$$.getSingle("div._full_1"); // 이미지 사이즈 조정 레이어
		this.imgWidth = jindo.$$.getSingle("input.widthimg"); // 가로 입력칸
		this.imgHeight = jindo.$$.getSingle("input.heightimg"); // 세로 입력칸
		this.btn = jindo.$$.getSingle("button.se2_sreset"); // 초기화 버튼
		this.tmpWidth = 0; // 초기화에 쓰일 가로 임시변수
		this.tmpHeight = 0; // 초기화에 쓰일 세로 임시변수
	},
	
	// 이미지 사이즈 조정 레이어의 이벤트들을 준비한다.
	$ON_MSG_APP_READY : function(){
		this.oApp.registerBrowserEvent(this.imgWidth, "keyup", "EVENT_UTIL_AREA_WIDTH_KEYUP"); // 가로 입력 시 이벤트
		this.oApp.registerBrowserEvent(this.imgHeight, "keyup", "EVENT_UTIL_AREA_HEIGHT_KEYUP"); // 세로 입력 시 이벤트
		this.oApp.registerBrowserEvent(this.btn, "click", "EVENT_UTIL_BUTTON_CLICK"); // 초기화 버튼 클릭 시 이벤트
	},
...
//		}
	...
	// 초기화 버튼 클릭 시 tmp 변수에 저장되있던 값들로 사이즈를 조정하는 함수
	// width 는 화면에 보이는 width
	// rwidth 는 글을 저장시 저장될 실제 width
	$ON_EVENT_UTIL_BUTTON_CLICK : function(){
		this.imgWidth.value = this.tmpWidth;
		this.imgHeight.value = this.tmpHeight;
		
		this.elImg.setAttribute("width", this.tmpWidth);
		this.elImg.setAttribute("height", this.tmpHeight);
		...
		if(this.elImg.style != null){
			// 크롬에서는 뒤에 "px" 를 붙여줘야 인식함
			this.elImg.style.width = this.tmpWidth + "px";
			this.elImg.style.height = this.tmpHeight + "px";
			this.elImg.style.rwidth = this.elImg.getAttribute("width");
			this.elImg.style.rheight = this.elImg.getAttribute("height");
		}
	},
	
	_hideLayer : function(){
		this.imgUtil.style.display = "none";
	},
	
	// 이미지 사이즈 조정 레이어에서 숫자를 제외한 값 입력시 지우는 함수
	_checkNumber : function(){
		var num_regx = /^[0-9]*$/;
		if(!num_regx.test(this.imgWidth.value)){
			alert('숫자만 입력 가능합니다.');
			this.imgWidth.value = this.imgWidth.value.replace(/[^0-9]/g, '');
		}
		if(!num_regx.test(this.imgHeight.value)){
			alert('숫자만 입력 가능합니다.');
			this.imgHeight.value = this.imgHeight.value.replace(/[^0-9]/g, '');
		}
	},
	
	// 화면에 보이는 사이즈와 실제 저장될 사이즈 입력하는 함수
	// width 는 화면에 보이는 width
	// rwidth 는 글을 저장시 저장될 실제 width
	_setWidthHeight : function(nAfterWidth, nAfterHeight){
		this.elImg.style.width = parseInt(nAfterWidth) + "px";
		this.elImg.style.height = parseInt(nAfterHeight) + "px";
		this.elImg.setAttribute("width", parseInt(nAfterWidth) );
		this.elImg.setAttribute("height", parseInt(nAfterHeight));
	...
	// 이미지 사이즈 조정 레이어에서 가로를 변경했을 때 세로값이 자동으로 변경되도록 하는 부분
	$ON_EVENT_UTIL_AREA_WIDTH_KEYUP : function(){
		this._checkNumber();// 숫자를 제외한 값 입력시 먼저 지운다.
		var nRatio, nAfterWidth, nAfterHeight, nWidthDiff, nHeightDiff, nborder;
		
		nborder = this.elImg.border || 0;
		nAfterWidth = this.imgWidth.value - (nborder*2);
		nAfterHeight = this.imgHeight.value - (nborder*2);
		...
		// 비율유지 체크 시 가로를 변경하면 세로에 소수점이 붙는 경우가 생기므로 정수화한다
		this.imgHeight.value = parseInt(nAfterHeight);
		
		// 화면에 보이는 사이즈와 실제 저장될 사이즈 입력
		this._setWidthHeight(nAfterWidth, nAfterHeight);
	},
	
	// 이미지 사이즈 조정 레이어에서 세로를 변경했을 때 가로값이 자동으로 변경되도록 하는 부분
	$ON_EVENT_UTIL_AREA_HEIGHT_KEYUP : function(){
		this._checkNumber();// 숫자를 제외한 값 입력시 먼저 지운다.
		var nRatio, nAfterWidth, nAfterHeight, nWidthDiff, nHeightDiff, nborder;
		...
		// 비율유지 체크 시 세로를 변경하면 가로에 소수점이 붙는 경우가 생기므로 정수화한다
		this.imgWidth.value = parseInt(nAfterWidth);
	...
	_attachResizeEnd : function(elImg){
		...
		// 이미지를 클릭 시이미지 사이즈 조정 레이어의  display 를 block 으로 한다.
		// 가로 세로 입력칸에 해당 이미지의 width, height 를 넣는다.
		this.imgUtil.style.display = "block";
		this.imgWidth.value = this.nWidth;
		this.imgHeight.value = this.nHeight;
		
		// tmpWidth 와 tmpHeight 에 각각 값을 입력해둔다.
		// 이 값들은 초기화 버튼 클릭 시 할당할 값들이다.
		// tmp 변수에 값이 저장되는 시기는 이미지 클릭시, 드래그로 이미지 사이즈 조정했을 때 두가지의 경우이다.
		this.tmpWidth = this.nWidth;
		this.tmpHeight = this.nHeight;
	},
