		...
		try{
			if( !oNavigator.ie ){
				if(!!oNavigator.safari && oNavigator.version <= 5){
					...
					//bSupportDragAndDropAPI = true;	//HTML5 부분 처리시까지 False
					bSupportDragAndDropAPI = false;
				}
			} else {
				bSupportDragAndDropAPI = false;
    ...
    function html5Upload() {	
    	...
    	sUploadURL= '/smarteditor/photo_uploader/popup/file_uploader_html5.asp'; 	//upload URL
	...
	function onAjaxError (){
 	...
 	function callFileUploader (){
 		//console.log("callFileUploader---->"+location.href);
 		oFileUploader = new jindo.FileUploader(jindo.$("uploadInputBox"),{
 			
 			//sUrl  : location.href.replace(/\/[^\/]*$/, '') + ''+document.forms[0].action,	//샘플 URL입니다.
 			sUrl  : document.forms[0].action,
...
//  				선택된 파일의 형식이 허용되는 경우만 처리 
 	    		if(oCustomEvent.bAllowed === true){
 	    			//alert('dfa1111'+location.href.replace(/\/[^\/]*$/, '') + ''+document.forms[0].action);
 		    		goStartMode();
 		    	}else{
 		    		//alert('dfa2222');
 		    		goReadyMode();
